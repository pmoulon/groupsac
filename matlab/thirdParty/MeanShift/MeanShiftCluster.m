function [clustCent,data2cluster,cluster2dataCell] = MeanShiftCluster(dataPts,bandWidth,sqdist)
...
bandSq          = bandWidth^2;
initPtInds      = 1:numPts;
stopThresh      = (1e-3*bandWidth) ^ 2;                %when mean has converged
...
while numInitPts
    ...
    while 1     %loop until convergence
        ...
        sqDistToAll = sqdist(dataPts, myMean);
        ...
        if sqdist(myMean, myOldMean) < stopThresh
            
            %check for merge posibilities
            mergeWith = 0;
            for cN = 1:numClust
                sqdistToOther = sqdist(myMean, clustCent(:,cN));  %distance from posible new clust max to old clust max
                if sqdistToOther < bandSq / 4                     %if its within bandwidth/2 merge new and old
                    mergeWith = cN;
                    break;
                end
            end
